...
echo "Collating registered region information with sparsity for fish in $input_dir"
...
do
    ...
    if [ -z "$2" ]
    then
        python ../cli_create_region_df_single_fish.py --input_path $fish_path
    else
        python ../cli_create_region_df_single_fish.py --input_path $fish_path --overwrite
    fi
done
