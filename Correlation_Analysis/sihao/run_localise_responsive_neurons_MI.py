...
from significance import compute_te_null, compute_stimulus_transfer_entropy, get_significant_neurons
...
# Compute mutual information for all neurons with respect to a given stimulus pattern
save_name = 'te_240823_FishH_nm' + str(stim_id) + '.npy'
transfer_entropies = compute_stimulus_transfer_entropy(traces_filtered, stim_info, stim_id, fs=2.18, n_lags=2, use_saved=False, save_name=save_name)

# Compute transfer entropy null distribution for all neurons with respect to a given stimulus pattern
null_save_name = 'transfer_entropy_null_240823_FishH_nm' + str(stim_id) + '.npy'
transfer_entropies_null = compute_te_null(traces_filtered, stim_info, stim_id, fs=2.18, n_lags=2, use_saved=False, save_name=null_save_name)

# Get significant neurons
significant_neurons, significant_neurons_mask = get_significant_neurons(transfer_entropies, transfer_entropies_null, alpha=0.01, coherence_threshold=0.016)

# Get highest mutual info values
highest_te = np.argsort(transfer_entropies)[-10:]

# Plot highest traces
fig, axs = plot_traces(traces, highest_te)
fig.show()


# Plot significant neurons with associated coherence values
fig, axs = plot_traces(traces, significant_neurons[:10])
# Overlay stimulus pattern
for i in range(10):
    axs[i].plot(stim_timeseries * np.max(traces[significant_neurons[:10]])/2, color='red')
