...
from utils import hoyer_sparsity, sort_responses
...
def main(input_path, fs=2.1802, overwrite=True):
    ...
    output_path = os.path.join(input_path, 'SVM_results')
    ...
    np.save(os.path.join(output_path, 'svm_neurons.npy'), neuron_selector)
    ...
    with open(os.path.join(output_path, 'svm.pkl'), 'wb') as f:
        pickle.dump(clf, f)
    ...
    # Save results
    os.makedirs(output_path, exist_ok=True)
    np.save(os.path.join(output_path, 'weights.npy'), weights)
    ...
    np.save(os.path.join(output_path, 'population_sparsity.npy'), population_sparsity)
    np.save(os.path.join(output_path, 'lifetime_sparsity.npy'), lifetime_sparsity)
    logger.info(f'Saved results to {output_path}')

    # Save summary plots
    fig, ax = plot_svm_results(confusion, lifetime_sparsity, population_sparsity, weights)
    fig.savefig(os.path.join(output_path, 'svm_summary_plot.svg'))
    ...
    # Cluster idx_to_plot using rastermap
    idx_sorted = sort_responses(traces_filtered_trials, idx_to_plot)

    # Plot mosaic of all highest responding neurons
    fig, ax = plot_trial_mosaic(traces_filtered_trials, idx_to_plot[idx_sorted], fs=fs, single_traces=True)
