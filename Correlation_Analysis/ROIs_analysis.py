...
wd = os.path.join('D:', 'Nico_data', 'scape_singles_dataset')
# name = '082423_fishN_run4_HR'
...
# roi = pd.read_csv(os.path.join(wd, name + '_Warped.csv'))
# roi_pool = pd.read_csv(os.path.join(wd, 'ROIs_pooled.csv'))

# ROIs: load, pool, save
roi_fn = sorted(list(glob.glob(os.path.join(wd, '*_Warped.csv'))))
...
ix0 = (roi_df['z'] >= 0) & (roi_df['z'] <= atlas.shape[0]) & \
    ...
    #   (roi_df['x'] >= 0) & (roi_df['x'] <= mon.shape[2] // 2)  # left side only
...
# ix1 = mon[roi_df.loc[ix0, 'z'].values.astype(int), 
#           roi_df.loc[ix0, 'y'].values.astype(int), 
#           roi_df.loc[ix0, 'x'].values.astype(int)].astype(bool)  # mon only
...
plt.show()
# plt.savefig(os.path.join(wd, 'figures', 'ROIs_heatmap_XYplane.svg'), dpi = 600)
...
ax.axis(False)
plt.show()
# plt.savefig(os.path.join(wd, 'figures', 'ROIs_heatmap_ZYplane.svg'), dpi = 600)
...
ax.axis(False)
plt.show()
# plt.savefig(os.path.join(wd, 'figures', 'ROIs_heatmap_XZplane.svg'), dpi = 600)
